...
class BaseTestOpenAPIv3Validator:

    @pytest.fixture
    def validator(self, spec):
        return openapi_v3_validator_factory.create(spec)


class BaseTestValidOpeAPIv3Validator(BaseTestOpenAPIv3Validator):

    def test_valid(self, validator, spec):
        return validator.validate(spec)


class BaseTestFailedOpeAPIv3Validator(BaseTestOpenAPIv3Validator):
